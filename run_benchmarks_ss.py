...
#TODO: fix Mask2Former ops for FLOPS calculation
def mean_iou(logits, masks_labels, num_out_classes, device, task:str = 'multiclass' ):
...
def batch_inference_template(model,
    ...
    Returns:
        A tuple containing the average loss and Mean IOU over the dataset.
    """
    # Test the model
    model.eval()
    ...
    batch_iou = sum(mious) / len(mious)

    return (batch_loss, batch_iou)
...
def test_model(model,
    ...
    Returns:
        test_loss: Loss of test data set
        test_iou: Test data set Intersection Over Union score ( compared from predicted vs actual segmentation map)
    """

    print("Testing model on test dataset")
    test_loss, test_iou = batch_inference_template(model = model,
...
def validate_model(model,
    ...
    Returns:
        val_loss: Loss of validation data set
        val_iou: validation data set Intersection Over Union score ( compared from predicted vs actual segmentation map)
    """
    
    print("Validating model on validation dataset")
    val_loss, val_iou = batch_inference_template(model = model,
...
def train_model(model,
    ...
    for index, (image_batch, label_batch) in tqdm( enumerate( iter(train_loader) ) , total = len(train_loader), desc = "Processing train batches"):
        ...
        if torch.cuda.is_available():
            ...
            with torch.amp.autocast(dtype = torch.bfloat16, device_type = "cuda"):
                ...
                # Alignment and Necessity ablations should always be turned off during training
                outputs = model(pixel_values = image_batch)
        
        # perform regular training on CPU
        else:
            outputs = model(pixel_values = image_batch)
    ...
    if val_model:
        val_loss, val_iou = validate_model(model,
                                           val_loader,
                                           loss_function,
                                           device,
                                           num_out_classes=num_out_classes,
        ...
        return (train_losses, val_loss, val_iou)
...
def setup_training(data_paths,
    ...
    for ep in tqdm( range(num_epochs) , desc = "Running epochs for training"):
        ...
        if ep % 2 == 0:
            train_loss, val_loss, val_iou = train_model(model = model,
    ...
    test_loss, test_iou = test_model(model,
                                     test_loader,
                                     criterion,
                                     device,
                                     num_out_classes=num_out_classes,
    ...
    if not local_testing:
        run_logger.log({"test_iou": test_iou, "test_loss": test_loss}); run_logger.finish()
...
if __name__ == "__main__":
    ...
    yaml_project_name = "vit_mask2form_semseg"; log_metrics = True; log_model = True
    
    configs_path = "./configs_train.yaml"
    data_paths = {"train_data": "/teamspace/gcs_folders/oxfordpet-train-cachegpu/train.pt",
                  "val_data" : "/teamspace/gcs_folders/oxfordpet-train-cachegpu/val.pt",
                  "test_data" : "/teamspace/gcs_folders/oxfordpet-train-cachegpu/test.pt" 
                  }
    ...
    if log_metrics:
        ...
        model_profile_dict['dataset']  = 'oxfordiiit_pet'
        run_logger.log({"model_profile": model_profile_dict})

    test_loss, test_iou = setup_training(data_paths = data_paths,
                   num_epochs = config_details['config']['num_epochs'],
                   model = model,
                   device=device,
                   batch_size = 512 if torch.cuda.is_available() else 64,
                   ...
                   log_model = log_model,
                   num_out_classes = config_details['config']['num_out'],
                   task = 'multiclass'
                   )
    
    print("Test Loss: ", test_loss)
    print("Test accuracy: ", test_iou)
