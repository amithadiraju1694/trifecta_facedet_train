...
from VIT_SemSeg_ablations import (
    ViTWithPEG_SemSeg,
    ViTLoRA_SemSeg,
...
def test_model(model,
    ...
    Returns:
        if topN is True, returns two floats (test_loss, test_metricuracy for top1).
...
def validate_model(model,
    ...
    Returns:
        if topN is True, returns two floats (validation_loss, validation_iou for top1).
    ...
    for index, (image_batch, label_batch) in tqdm( enumerate( iter(train_loader) ) , total = len(train_loader), desc = "Processing train batches"):
        ...
        if torch.cuda.is_available():
            ...
            with torch.amp.autocast(dtype = torch.bfloat16, device_type = "cuda"):
                ...
                outputs = model(pixel_values = image_batch, alignment_mode = False, necs_mode = False)
...
def setup_training(data_paths,
    ...
    for ep in tqdm( range(num_epochs) , desc = "Running epochs for training"):
        ...
        if ep % 2 == 0:
            train_loss, val_loss, val_metric = train_model(model = model,
    ...
    test_loss, test_metric = test_model(model,
                                     test_loader,
                                     criterion,
                                     device,
                                     inf_algn_mode,
...
if __name__ == "__main__":
    ...
    if log_metrics:
        ...
        model_profile_dict['dataset']  = 'oxford_iitpet'
        run_logger.log({"model_profile": model_profile_dict})

    test_loss, test_metric = setup_training(data_paths = data_paths,
                   num_epochs = config_details['config']['num_epochs'],
                   model = model,
                   device=device,
                   batch_size = 512 if torch.cuda.is_available() else 64,
